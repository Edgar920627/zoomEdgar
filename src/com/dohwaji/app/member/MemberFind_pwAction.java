...
public class MemberFind_pwAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// id 확인
		check_id = m_dao.checkId(user_id);
		...
		if(check_id == false) {
			...
			out.println("<script>");
			out.println("alert('회원가입된 아이디가 없습니다.');history.back();");
			out.println("</script>");
			out.close();		
			return null;
		}
		...
		// 이메일 제목
		String subject = ".zoom. 비밀번호 찾기  이메일 인증 해주세요. ";
		
		// 이메일 내용
		String content = "다음 링크에 접속하여 이메일 확인을 진행하세요. " +
				"<a href='" + host + "member/Find_PWEmailCheck.me?user_email=" + user_email +"&code=" + new SHA256().getSHA256(to) + "'>이메일 인증하기</a>";
